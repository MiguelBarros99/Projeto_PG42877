...
# # ### Utilizado para Reduzir repetições
DIC_S = {}
PDBf = []
for i in dic.keys():
    xi = dic[i].split(' ')
...
f = open('textfiles/reddata.txt', "w+")
for i in DIC_S.keys():
...
k = open('textfiles/reddata.txt', "r")
...
f = open('textfiles/redudres.txt', "w+")
for i in DIC_F.keys():
...
#PDB -> UNIPROT
f = open('textfiles/redudres.txt', "r")
x = f.readlines()
FILE = {}
contar = []
for i in x:
...
def dataset(dic_s, PDB_EC):
    ...
    for i in dic_s.keys():
        ...
        if i in PDB_EC:
            ...
            # print(i + ' in progress...')
            res = Histogram(i, dic_s[i], bin= [32,32])
