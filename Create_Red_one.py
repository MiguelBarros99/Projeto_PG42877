...
for i in dic.keys():
    print( i + ' in progress...')
    sel = PDB_info(dic[i], i,auto= False)
    sel = sel.PDB_selection()
    if sel != 'ERRO':
        DIC_S[i] = sel
        if sel not in PDBf: PDBf.append(sel)

print(' UNIPROTS: '+ str(len(DIC_S.keys())) + ' PDBs: ' + str(len(PDBf)))

f = open('textfiles/Red_one.txt', "w+")
for i in DIC_S.keys():
...
k = open('textfiles/Red_one.txt', "r")
kx = k.readlines()
dic = {}
for i in kx:
    xi = i.split()
...
f = open('textfiles/Res_one.txt', "w+")
for i in DIC_F.keys():
...
df = pd.read_csv('textfiles/ECnumber_39251.csv')
IDs_39251= df['Entry']
ECs = df['EC number']
for i in range(len(ECs)):
...
#PDB -> UNIPROT
f = open('textfiles/Res_one.txt', "r")
x = f.readlines()
FILE = {}
contar = []
for i in x:
